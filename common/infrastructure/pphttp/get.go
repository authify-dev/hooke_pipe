package pphttp

import (
	"common/domain/customctx"
	"common/domain/logger"
	pphttp "common/infrastructure/pphttp/errs"
	"common/utils"
	"common/utils/pperrs"
	"encoding/json"
	"io"
	"net/http"
	"net/url"
	"strconv"
	"strings"
)

func encodeRequestPath(rawPath string) string {
	// Separar path de query
	parts := strings.SplitN(rawPath, "?", 2)

	if len(parts) == 1 {
		// No hay query params
		return rawPath
	}

	path := parts[0]
	rawQuery := parts[1]

	// Parsear los query params
	values, err := url.ParseQuery(rawQuery)
	if err != nil {
		// Si hay error al parsear, devolver el path tal cual
		return rawPath
	}

	// Encode y luego reemplazar '+' por '%20'
	encodedQuery := values.Encode()
	encodedQuery = strings.ReplaceAll(encodedQuery, "+", "%20")

	// Re-encodearlos
	return path + "?" + encodedQuery
}

func (c *CustomApiClient) GET(
	ctx *customctx.CustomContext,
	request CustomApiRequest,
) utils.Result[CustomApiResponse] {

	url := c.base_url + encodeRequestPath(request.Path)
	entry, done := logger.FromContextWithExit(ctx.Context())
	defer done()
	entry.Info("Make GET request to ", url)

	if request.Debug {
		logger.InfoD(entry, "Headers: ", request.Headers)
	}

	req, err := http.NewRequestWithContext(ctx.Context(), http.MethodGet, url, nil)
	if err != nil {
		entry.Error("Error creating request", err)
		return utils.Result[CustomApiResponse]{
			Err: ctx.NewError(pphttp.NewMakingRequestError(pphttp.RequestOptions{
				Method: http.MethodPost,
				URL:    url,
			})),
		}
	}

	c.setHeaders(req, request.Headers)

	resp, err := c.http_client.Do(req)
	if err != nil {
		entry.Error("Error making GET request", err)
		return utils.Result[CustomApiResponse]{
			Err: ctx.NewError(pphttp.NewDoingRequestError(pphttp.RequestOptions{
				Method: http.MethodGet,
				URL:    url,
			})),
		}
	}

	defer resp.Body.Close()

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		entry.Error("Error reading the response", err)
		return utils.Result[CustomApiResponse]{
			Err: ctx.NewError(pphttp.NewReadingResponseError(pphttp.RequestOptions{
				Method: http.MethodPost,
				URL:    url,
			})),
		}
	}

	if request.Debug {
		logger.InfoD(entry, "Response Body: ", string(body))
	}

	var _err *pperrs.CustomError

	if resp.StatusCode != request.ExpectedCode {
		entry.Errorf("Error inesperado: %d — %s", resp.StatusCode, string(body))
		_err = &pperrs.CustomError{
			Code:    500,
			Message: "error inesperado",
			Scope:   "konectus.knhttp.POST",
		}
	}

	var result map[string]any

	err = json.Unmarshal(body, &result)
	if err != nil {
		entry.Errorf("Error al decodificar JSON: %v", err)
		_err = &pperrs.CustomError{
			Code:    500,
			Message: "error al decodificar JSON",
			Scope:   "konectus.knhttp.POST",
		}
	}

	if _err != nil {
		return utils.Result[CustomApiResponse]{
			Data: CustomApiResponse{
				Data:       result,
				StatusCode: strconv.Itoa(resp.StatusCode),
			},
			Err: ctx.NewError(_err),
		}
	}

	return utils.Result[CustomApiResponse]{
		Data: CustomApiResponse{
			Data:       result,
			StatusCode: strconv.Itoa(resp.StatusCode),
		},
	}
}
