...
func (r *PostgresRepository[E, M, Q]) rowsToEntity(columns []string, rows *sql.Rows) utils.Result[E] {
	...
	if err := rows.Scan(pointers...); err != nil {
		return utils.Result[E]{Err: pperrs.NewCustomError(http.StatusInternalServerError, err.Error(), "postgres.matching_low.scan")}
	}
	...
	if err != nil {
		return utils.Result[E]{Err: pperrs.NewCustomError(http.StatusInternalServerError, err.Error(), "postgres.matching_low.from_json")}
	}
