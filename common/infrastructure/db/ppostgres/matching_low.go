...
func (r *PostgresRepository[E, M, Q]) MatchingLow(cr criteria.Criteria, table_name string, offset int, limit int) utils.Result[[]E] {
	...
	if err != nil {
		return utils.Result[[]E]{Err: pperrs.NewCustomError(http.StatusInternalServerError, err.Error(), "postgres.matching_low.get_rows")}
	}
	defer rows.Close()
	...
	if err != nil {
		return utils.Result[[]E]{Err: pperrs.NewCustomError(http.StatusInternalServerError, err.Error(), "postgres.matching_low.get_columns")}
	}
	...
	if err := rows.Err(); err != nil {
		return utils.Result[[]E]{Err: pperrs.NewCustomError(http.StatusInternalServerError, err.Error(), "postgres.matching_low.get_rows_err")}
	}
