...
func (m *MongoRepository[T]) Save(ctx context.Context, document T) utils.Result[string] {
	...
	if err != nil {
		return utils.Result[string]{Err: pperrs.NewCustomError(http.StatusInternalServerError, err.Error(), "mongo.save")}
	}

	// Convertir el InsertedID a primitive.ObjectID
	insertedID, ok := result.InsertedID.(primitive.ObjectID)
	if !ok {
		return utils.Result[string]{Err: pperrs.NewCustomError(http.StatusInternalServerError, "el InsertedID no es un ObjectID válido", "mongo.save")}
	}
...
func (m *MongoRepository[T]) SaveWithID(ctx context.Context, id string, document T) utils.Result[string] {
	...
	if err != nil {
		return utils.Result[string]{Err: pperrs.NewCustomError(http.StatusInternalServerError, err.Error(), "mongo.save_with_id")}
	}
...
func (m *MongoRepository[T]) Update(ctx context.Context, entity T) error {
	...
	if err != nil {
		return pperrs.NewCustomError(http.StatusInternalServerError, err.Error(), "mongo.update")
	}
	...
	if result.MatchedCount == 0 {
		return pperrs.NewCustomError(http.StatusInternalServerError, fmt.Errorf("no se encontró documento con id %s", id).Error(), "mongo.update")
	}
...
func (m *MongoRepository[T]) UpdateFields(ctx context.Context, id string, updates map[string]interface{}) utils.Result[T] {
	...
	if err != nil {
		...
		return utils.Result[T]{Err: pperrs.NewCustomError(http.StatusInternalServerError, fmt.Errorf("id inválido (%s): %w", id, err).Error(), "mongo.update_fields")}
	}

	// 2. Validar campos a actualizar
	if len(updates) == 0 {
		entry.Error("No se proporcionaron campos para actualizar")
		return utils.Result[T]{Err: pperrs.NewCustomError(http.StatusInternalServerError, fmt.Errorf("no se proporcionaron campos para actualizar").Error(), "mongo.update_fields")}
	}
	...
	if err != nil {
		...
		if err == mongo.ErrNoDocuments {
			...
			return utils.Result[T]{Err: pperrs.NewCustomError(http.StatusInternalServerError, fmt.Errorf("no se encontró ningún documento con id %s", id).Error(), "mongo.update_fields")}
		}
		return utils.Result[T]{Err: pperrs.NewCustomError(http.StatusInternalServerError, fmt.Errorf("error al devolver el documento actualizado: %w", err).Error(), "mongo.update_fields")}
	}
...
func (m *MongoRepository[T]) Delete(ctx context.Context, id string) error {
	...
	if err != nil {
		return pperrs.NewCustomError(http.StatusInternalServerError, fmt.Errorf("error al eliminar el documento: %w", err).Error(), "mongo.delete")
	}

	// Verificar que se haya eliminado algún documento
	if result.DeletedCount == 0 {
		return pperrs.NewCustomError(http.StatusInternalServerError, fmt.Errorf("no se encontró documento con id %s", id).Error(), "mongo.delete")
	}
...
func (m *MongoRepository[T]) Find(ctx context.Context, id string) utils.Result[T] {
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			return utils.Result[T]{Err: pperrs.NewCustomError(http.StatusInternalServerError, fmt.Errorf("no se encontró documento con id %s", id).Error(), "mongo.find")}
		}
		return utils.Result[T]{Err: pperrs.NewCustomError(http.StatusInternalServerError, fmt.Errorf("error al buscar el documento: %w", err).Error(), "mongo.find")}
	}
