...
func (r *PostgresRepository[E, M]) Save(role E) utils.Result[string] {
	...
	if err := r.Connection.Create(&roleModel).Error; err != nil {
		...
		if errors.As(err, &pgErr) && pgErr.Code == "23505" {
			...
			return utils.Result[string]{Err: pperrs.NewCustomError(http.StatusBadRequest, err.Error(), "postgres.save.duplicate_key")}
		}
		return utils.Result[string]{Err: pperrs.NewCustomError(http.StatusInternalServerError, err.Error(), "postgres.save.error")}
	}
