...
func EntityToModel[E IEntity, M IModel](entity IEntity) utils.Result[M] {
	...
	if err != nil {
		return utils.Result[M]{Err: pperrs.NewCustomError(http.StatusInternalServerError, err.Error(), "ientity.entity_to_model.Marshal")}
	}

	// Convertir los bytes JSON a un mapa.
	err = json.Unmarshal(data, &result)
	if err != nil {
		return utils.Result[M]{Err: pperrs.NewCustomError(http.StatusInternalServerError, err.Error(), "ientity.entity_to_model.Unmarshal")}
	}

	// Convertir el mapa a modelo.
	model, err := ModelFromJSON[M](result)
	if err != nil {
		return utils.Result[M]{Err: pperrs.NewCustomError(http.StatusInternalServerError, err.Error(), "ientity.entity_to_model.ModelFromJSON")}
	}
